...
/*
Core of the Themple Framework. Both Theme and Lite version needs to load it
*/
...
// The options array that stores all the options used in the theme
...
// Gets the path directory for the plugin or theme root
function tpl_base_dir() {
	if ( defined( 'THEMPLE_THEME' ) ) {
		if ( THEMPLE_THEME == true ) {
...
// Gets the URI for the plugin or theme root
...
function tpl_register_option ( $narr ) {
	...
	if ( !isset( $narr["name"] ) ) {
		tpl_error(
			apply_filters( 'tpl_option_noname_errormsg', __( 'You forgot to set name for an option. Please check your options files.', 'themple' ) ), true
		);
		return;
	}
...
function tpl_register_section ( $narr ) {
	...
	// If no post_type was set, it goes automatically to the Theme Options page
...
// Checks if this is primary (Theme Options, Framework Options, etc.) or secondary (e.g. Post Metabox) section
...
// General function for creating Themple settings pages in the back end
...
function tpl_admin_scripts() {
	...
	wp_enqueue_script( 'tpl-admin-scripts', tpl_base_uri() . '/framework/script/admin-scripts.js', array( 'jquery', 'jquery-ui-tabs' ), THEMPLE_VERSION );
	wp_enqueue_style( 'tpl-admin-style', tpl_base_uri() . '/framework/style/admin/css/admin.css', array(), THEMPLE_VERSION );

	wp_localize_script( 'tpl-admin-scripts', 'Themple_Admin', array_merge( apply_filters( 'tpl_admin_js_strings', array() ), tpl_admin_vars_to_js() ) );
