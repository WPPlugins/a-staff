...
class TPL_Combined extends TPL_Data_Type {
	...
	public function form_field_content ( $for_bank = false ) {
		...
		echo '<div class="tpl-combined-wrapper tpl-datatype-container">';
			...
			if ( $part->repeat !== false ) {
				...
				if ( !isset( $part->path[$path_s+1] ) || $part->path[$path_s+1] == 0 ) {
					...
					if ( isset( $part->description ) && $part->description != '' ) {
						echo '<i class="fa fa-lg fa-question-circle tpl-admin-question tpl-admin-icon"' . $data_connected . '><span class="tpl-hovermsg"><span class="tpl-hovermsg-inner">' . tpl_kses( $part->description ) . '</span></span></i>';
					}
